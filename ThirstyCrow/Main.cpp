...
float theta = 0.0;
int x = 0, m = 0, y = 0, g = 0, h = 0, j = 0, k = 0, o = 0, p = 0, r = 0, s = 0, u = 0, v = 0 , q = 0,t = 0;
...
void welcomeDisplay()  // First display of the project
{
    ...
    glColor3f(0.0, 0.0, 0.0);
    glRasterPos3f(-140.0, 250.0, 0);
    char message3[] = "DESCRIPTION : ";
    ...
    glColor3f(0.0, 0.0, 0.0);
    glRasterPos3f(-500.0, 200.0, 0);
    char message4[] = "GRAPHICAL DEPICTION OF THE STORY MENTIONED ABOVE.";
    ...
    glColor3f(0.0, 0.0, 0.0);
    glRasterPos3f(-500.0, -150.0, 0);
    char message9[] = "PROJECT BY :";
    ...
    char message5[] = "PRAJNA G - MY.SC.U3BCA21125";
    for (int k = 0; k < strlen(message5); k++)
    {
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, message5[k]);
    }

    glRasterPos3f(-250.0, -200.0, 0);
    char message6[] = "SHREYA M DESAI - MY.SC.U3BCA21132";
    for (int k = 0; k < strlen(message6); k++)
    {
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, message6[k]);
    }
    
    glRasterPos3f(-250.0, -250.0, 0);
    char message11[] = "ANJALLI AGARWAL - MY.SC.U3BCA21115";
    for (int k = 0; k < strlen(message11); k++)
    {
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, message11[k]);
    }

    glRasterPos3f(-250.0, -300.0, 0);
    char message10[] = "NIKITHA ANANTHAKRISHNA - MY.SC.U3BCA21083";
...
void storypage()
{
    ...
    glColor3f(0.0, 0.0, 0.0);
    glRasterPos3f(-200.0, 480.0, 0);
    char message1[] = "STORY: THIRSTY CROW";
    for (int k = 0; k < strlen(message1); k++)
    {
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, message1[k]);
    }
    ...
    glRasterPos3f(-920.0, 400.0, 0);
    char message2[] = "It was a hot summer day. A thirsty crow was flying in search of water in the desert. The crow flew over the desert.";
    for (int k = 0; k < strlen(message2); k++)
    {
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, message2[k]);
    }

    glRasterPos3f(-920.0, 350.0, 0);
    char message3[] = "But he didn't find any water. After a long time, he came across a tree. There was a pot of water in the desert.";
    for (int k = 0; k < strlen(message3); k++)
    {
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, message3[k]);
    }

    glRasterPos3f(-920.0, 250.0, 0);
    char message4[] = "Happy that he found some water finally, he swooped down from the tree to the pot on the ground. He quickly peeked ";
    for (int k = 0; k < strlen(message4); k++)
    {
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, message4[k]);
    }

    glRasterPos3f(-920.0, 200.0, 0);
    char message5[] = "inside the pot . There was very little water in the pot. The crow put his beak inside the pot but could not ";
    for (int k = 0; k < strlen(message5); k++)
    {
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, message5[k]);
    }

    glRasterPos3f(-920.0, 150.0, 0);
    char message6[] = "reach the water.The water level was too low, and the narrow opening prevented his neck from going all the way down.";
    for (int k = 0; k < strlen(message6); k++)
    {
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, message6[k]);
    }

    glRasterPos3f(-920.0, 50.0, 0);
    char message7[] = " The crow was disappointed as he was really thirsty and needed a drink of water badly. ";
    for (int k = 0; k < strlen(message7); k++)
    {
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, message7[k]);
    }

    glRasterPos3f(-920.0, -0.0, 0);
    char message8[] = "He could have given up and flew away. But he didn't. Instead, he looked around and thought";
    for (int k = 0; k < strlen(message8); k++)
    {
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, message8[k]);
    }

    glRasterPos3f(-920.0, -100.0, 0);
    char message9[] = "What else can I do? He saw that there were a lot of pebbles on the ground. And, he came up with an idea!";
    for (int k = 0; k < strlen(message9); k++)
    {
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, message9[k]);
    }
    ...
    glRasterPos3f(-920.0, -150.0, 0);
    char message10[] = "He then collected pebbles one by one and put it into the pot. He collected another pebble and another and  ";
    for (int k = 0; k < strlen(message10); k++)
    {
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, message10[k]);
    }
    ...
    glRasterPos3f(-920.0, -200.0, 0);
    char message11[] = "put them all in the pot. As he added more and more stones, the water level came up to the brim. when the water level came up,";
    for (int k = 0; k < strlen(message11); k++)
    {
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, message11[k]);
    }
    ...
    glRasterPos3f(-920.0, -250.0, 0);
    char message12[] = "the crow drank the water and flew away happily. ";
    for (int k = 0; k < strlen(message12); k++)
    {
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, message12[k]);
    }
...
void lesswaterpot()
{
    ...
    {
        theta = i * 3.142 / 180;
        glVertex2f(0 + 20 * cos(theta), 0 + 35 * sin(theta));
    }
    glEnd();
}
...
void fullwaterpot()
{
    ...
    {
        theta = i * 3.142 / 180;
        glVertex2f(0 + 45 * cos(theta), 0 + 50 * sin(theta));
    }
    glEnd();
...
void emptywaterpot()
{
    ...
    {
        theta = i * 3.142 / 180;
        glVertex2f(0 + 20 * cos(theta), 0 + 25 * sin(theta));
    }
    glEnd();
    glColor3f(0.35, 0.35, 0.35);
    glBegin(GL_POLYGON); // stone 1
...
void crow(void)
{
    ...
    glBegin(GL_TRIANGLES);  // crow beak
    glVertex2f(10, 15);
    glVertex2f(40, 10);
    glVertex2f(10, -15);
    glEnd();
...
void crowstone()
{
    ...
    glColor3f(0.35, 0.35, 0.35);
    glBegin(GL_POLYGON); // stone in crow beak
    for (int i = 0; i < 360; i++)
    {
        theta = i * 3.142 / 180;
...
void movement7()  // movement of crow flying away
{
    glClear(GL_COLOR_BUFFER_BIT);
    glPushMatrix();
    display();
    ...
    glPopMatrix();
    glPushMatrix();
    glTranslated(-40.0 + q, -199.0 + t, 0.0);
    reversecrow();
    glPopMatrix();
    q -= 114;
    t += 99;
...
void keyAction(unsigned char key, int x, int y)
{
    ...
    case '8':  // for upward movement
        ...
        break;
    case 'd': // to show crow drank the water
        for (m = 0; m <= 5; m++)
        {
            checkpot3();
        }
